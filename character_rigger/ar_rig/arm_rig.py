...
class arm_rig():
    ...
    def arm_rig(self, 
        ...
        for i in main_arm_jnts:
            fk_jnts = 'FK_' + i
            main_fk_arm_jnts.append(fk_jnts)

        main_ik_arm_jnts = []
        for i in main_arm_jnts:
            ik_jnts = 'IK_' + i
            main_ik_arm_jnts.append(ik_jnts)
        ...
        ikHandle_var = mc.ikHandle(n=main_ik_arm_jnts[1] + '_ikHandkle', sj=main_ik_arm_jnts[1], ee=main_ik_arm_jnts[-1])
        ...
        for i in range(0,1):
            ...
            #rename curve
            myCurve = mc.rename(main_ik_arm_jnts[-1] + '_ik_ctrl')
            #group curve
            ...
            #parent and zero curveGrp to joints
            mc.parent(myGroup, main_ik_arm_jnts[-1], relative=True)
            #unparent group (since it has correct position)
            mc.Unparent(myGroup)
        ...
        for i in range(0,1):
            ...
            #rename curve
            myCurve = mc.rename(main_ik_arm_jnts[0] + '_ik_ctrl')
            #group curve
            curveGrouped = mc.group(myCurve)
            curveGrouped_offset = mc.group(myCurve)
            #rename group
            ...
            myGroup_offset = mc.rename(curveGrouped_offset, (myCurve + '_grp_offset'))
            #parent and zero curveGrp to joints
            mc.parent(myGroup, main_ik_arm_jnts[0], relative=True)
            #unparent group (since it has correct position)
            mc.Unparent(myGroup)
        ...
        # parent constrain clav joint translation to control
        mc.parentConstraint(ik_clav_ctrl_list[0], main_ik_arm_jnts[0])
        #scale constrain ctrl to jnt
        mc.scaleConstraint(ik_clav_ctrl_list[0], main_ik_arm_jnts[0])
