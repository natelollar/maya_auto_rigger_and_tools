...
def tentacle_rig(   defaultJnt_prefix = 'baseJnt_', 
    ...
    ikSpline_jntPos_lst = [tentacleStart_jnt, tentacleEnd_jnt]
    ...
    mc.setAttr(crv_len_ratio + '.input2X', curveInfo_length)
...
def tentacle_rig_bounding_boxes():
    parent_ctrl_name = 'standin_obj_tentacle_parent'
    parent_ctrl_grp = parent_ctrl_name + '_grp'

    start_ctrl_name = 'standin_obj_tentacle_start'
    start_ctrl_grp = start_ctrl_name + '_grp'

    end_ctrl_name = 'standin_obj_tentacle_end'
    end_ctrl_grp = end_ctrl_name + '_grp'

    parent_curve_name = 'parent_tentacle_curve'

    start_curve_name = 'start_tentacle_curve'

    organize_grp_name = 'bb_tentacle_grp'

    #____________________#
    if mc.objExists(organize_grp_name) == False:
        organize_grp = mc.group(em=True, n='bb_tentacle_grp')

    #____________________#
    if mc.objExists(parent_ctrl_name) == False:
        bb_nurbs_ctrl.bb_nurbs_ctrl(name='standin_obj_tentacle_parent', 
                                    size=5, 
                                    color1R=0, 
                                    color1G=1, 
                                    color1B=1, 
        ...
        mc.select(parent_ctrl_name)
        mc.Unparent()
        mc.delete(parent_ctrl_grp)

        mc.parent(  parent_ctrl_name, 
                    organize_grp)

    
    #____________________#
    if mc.objExists(start_ctrl_name) == False:
        bb_nurbs_ctrl.bb_nurbs_ctrl(name='standin_obj_tentacle_start', 
                                    size=5, 
                                    color1R=0, 
                                    color1G=1, 
                                    color1B=0, 
        ...
        mc.setAttr(start_ctrl_name + '.translate', 0, 140, 0)
    ...
    if mc.objExists(end_ctrl_name) == False:
        bb_nurbs_ctrl.bb_nurbs_ctrl(name='standin_obj_tentacle_end', 
                                    size=5, 
                                    color1R=1, 
                                    color1G=0, 
                                    color1B=0, 
        ...
        mc.setAttr(end_ctrl_name + '.translate', 0, 100, -285)
    ...
    if mc.objExists(parent_curve_name) == False:
        parent_curve = mc.curve(degree=1, p=[(0,0,0),(0,0,-100)], n='parent_tentacle_curve')
        parent_curve_shp = mc.listRelatives(parent_curve, type='shape')
        mc.setAttr(parent_curve_shp[0] + '.overrideEnabled', 1)
        mc.setAttr(parent_curve_shp[0] + '.overrideRGBColors', 1)
        mc.setAttr(parent_curve_shp[0] + '.overrideColorRGB', 0, 1, 1)
        mc.setAttr(parent_curve_shp[0] + '.lineWidth', 2)

        mc.connectAttr( parent_ctrl_name + '.translate', 
                        parent_curve_shp[0] + '.controlPoints[0]', f=True )

        mc.connectAttr( start_ctrl_name + '.translate', 
                        parent_curve_shp[0] + '.controlPoints[1]', f=True )
    ...
    if mc.objExists(start_curve_name) == False:
        start_curve = mc.curve(degree=1, p=[(0,0,0),(0,0,-100)], n='start_tentacle_curve')
        ...
        mc.setAttr(start_curve_shp[0] + '.overrideColorRGB', 0, 1, 1)
        mc.setAttr(start_curve_shp[0] + '.lineWidth', 2)
