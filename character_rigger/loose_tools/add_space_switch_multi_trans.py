...
def add_space_switch_multi_trans():
    ...
    trans_prnt_const_nm = constrained_obj + "_trans" + "_parentConstraint1"
    trans_prnt_const = cmds.parentConstraint( constraining_objs, constrained_obj, n=trans_prnt_const_nm, sr=("x", "y", "z"), mo=True, w=0 ) 
    ...
    cmds.addAttr( constrained_obj, 
                ln='trans_Space_Switch', 
                nn='trans_Space_Switch', 
                at='enum', 
                k=1, 
    ...
    space_swtch_attr = constrained_obj + '.trans_Space_Switch'
    ...
    for objects in constraining_objs:
        prnt_cnst_attr = trans_prnt_const[0] + '.' + objects + 'W' + str(current_ind)
    ...
    cmds.connectAttr(attribute_object + '.trans_Space_Switch', constrained_obj + '.trans_Space_Switch', force=True)
